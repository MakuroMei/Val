...
const player={hp:0x40,maxHp:0x40,mp:0x20,maxMp:0x20,poison:false,terrainMode:'land',xp:0,gold:0,equippedItem:0x0B,inventory:new Uint16Array(0x1D),hurtBlink:0,dead:false};
...
function dispatchSpecialTerrain(s,dir){
  ...
  if(t>=0xDC&&t<=0xDF){say('BRIDGE EVENT · inventory action pending',75);return false}
  if(t>=0xE0&&t<=0xE4)return false;
  if(t===0xE5||t===0xE7){say('ENDING RELIC GATE',75);return false}
  if(t===0xE8)return false;if(t===0xE9){say('OPEN WATER · Magic Ship / Marco required',75);return false}
  ...
  if(t>=0xF0&&t<=0xF3){say('KEY GATE · Key action pending',75);return false}
...
function beginAttack(){if(player.dead||attackTimer!==0)return;attackTimer=16;attackHitActive=true;moving=false}
...
const simpleBase={
  ...
  0x3A:0x61,0x41:0xCD,0x42:0xED,0x43:0xFD,0x45:0xC8,0x46:0xEC
};
const singleSimple=new Set([0x27,0x28,0x29,0x2A,0x2B,0x2C,0x2D,0x2E,0x2F,0x30,0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39]);
...
function playerAttackMeta(){const second=attackTimer<8;if(facing===0)return{id:second?0xC3:0xC2,flip:false};if(facing===1)return{id:second?0xC1:0xC0,flip:false};return{id:second?0xC5:0xC4,flip:facing===2}}
function drawPlayer(){
...
function initActorFromSpawnToken(token,x,y){
  const rec=token&0x3F,cls=VDATA.combatClass[rec]||0;if(cls===0)return false;
  const slot=firstFreeActorSlot();if(slot<0)return false;if(actorRequiresGroundCheck(cls)&&!terrainPassableForActor(x,y))return false;
  const dir=(rng8()%8)+1,hp=VDATA.combatHp[rec]||1;
  actors[slot]={kind:'combat',cls,rec,pal:VDATA.combatPalette[rec]||0,x,y,dir,desired:dir,timer:1,age:0,logicFrame:0,hp,maxHp:hp,power:VDATA.combatPower[rec]||0,xp:VDATA.combatXp[rec]||0,hitTimer:0,deathTimer:0,aimedInterval:false,zounaTimer:0xFF,zounaVisible:true};return true;
}
...
function finalizeDefeat(a){
  ...
  a.kind='item';a.cls=1;a.itemId=itemId;a.pal=ITEM_SPRITE_ATTR[itemId]&3;a.meta=ITEM_WORLD_META[itemId]||0x44;a.hitTimer=0;a.deathTimer=0;a.age=32;
...
function tryPickupItem(a,i){
  ...
  else{player.inventory[a.itemId]++;say(`PICKUP ${ITEM_NAMES[a.itemId]} ×${player.inventory[a.itemId]}`,70)}actors[i]=null;
}
...
function spawnShooterTest(){
  ...
  if(initActorFromSpawnToken(rec,p.x+56,p.y)){const a=actors.find(x=>x&&x.kind==='combat'&&x.rec===rec&&x.age===0);if(a){a.age=32;a.logicFrame=0;a.timer=1}say(`DEV SPAWN ${VDATA.combatLabel[rec]} · F3 NEXT`,75)}else say('DEV SPAWN FAILED · actor slots full',75);
}
...
function update(){
  frame++;if(mode!=='game'){pressed.clear();return}frameCounter=(frameCounter+1)&255;updateWorldClock();updatePlayer();tryPeriodicCellEncounterWave();updateActors();
  attackHitActive=false;if(attackTimer>0)attackTimer--;if(player.hurtBlink>0)player.hurtBlink--;if(noticeTimer>0)noticeTimer--;pressed.clear();
}
...
function drawWorld(){
  ...
  const alive=actors.reduce((n,a)=>n+(a&&a.kind==='combat'?1:0),0),shots=actors.reduce((n,a)=>n+(a&&a.kind==='projectile'?1:0),0);ctx.fillText(`HP ${player.hp}/${player.maxHp}  XP ${player.xp}  G ${player.gold}`,8,226);ctx.fillText(`${realm().toUpperCase()}  ENM ${alive} SHOT ${shots}  DMG ${weaponDamage()}  S4`,8,236);
  ...
  if(debug){
    ...
    ctx.fillText(`PATCH ${terrainPatches.size} LOCK ${countLocks(encounterLocks[realm()])}`,8,54);ctx.fillText(`POISON ${player.poison?1:0} INV ${player.inventory.reduce((a,b)=>a+b,0)}`,8,64);
  }
}
...
function start(force=false){
  ...
  frameCounter=0xFF;worldClockSubsecond=0;worldClockSecond=0;worldDay=0;rngState=0xA7;player.hp=player.maxHp=0x40;player.mp=player.maxMp=0x20;player.poison=false;player.terrainMode='land';player.xp=0;player.gold=0;player.equippedItem=0x0B;player.inventory.fill(0);player.inventory[0x0B]=1;player.hurtBlink=0;player.dead=false;
  terrainPatches.clear();encounterLocks.surface.fill(0);encounterLocks.dungeon.fill(0);clearActors();notice='';noticeTimer=0;pressed.clear();keys.clear();
}

addEventListener('keydown',e=>{
  if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space'].includes(e.code))e.preventDefault();
  if(e.code==='Enter'){if(mode==='title')start();else if(player.dead)start(true);return}
  if(e.code==='KeyR'){mode='title';keys.clear();pressed.clear();return}
...
for(const b of document.querySelectorAll('[data-key]')){
  ...
  const on=e=>{e.preventDefault();if(e.pointerId!=null&&b.setPointerCapture){try{b.setPointerCapture(e.pointerId)}catch{}}if(!keys.has(k))pressed.add(k);keys.add(k)};
  const off=e=>{e.preventDefault();keys.delete(k);if(e.pointerId!=null&&b.hasPointerCapture&&b.hasPointerCapture(e.pointerId)){try{b.releasePointerCapture(e.pointerId)}catch{}}};
...
addEventListener('blur',()=>{keys.clear();pressed.clear()});document.addEventListener('visibilitychange',()=>{if(document.hidden){keys.clear();pressed.clear()}});
})();
